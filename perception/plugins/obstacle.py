...
class LocalDistanceAdapter:
    ...
    def __init__(self, cfg: dict, progress_cb=None):
        ...
        engine_paths = ensure_obstacle_models(
            self._cfg.get("model_dir", "/models/obstacle/yolo26s-metric"),
...
def _build_distance_adapter(cfg: dict, progress_cb=None) -> LocalDistanceAdapter:
    if cfg.get("provider", "local") != "local":
        raise ValueError("obstacle provider must be local")
    return LocalDistanceAdapter(cfg, progress_cb=progress_cb)
...
class ObstacleDistancePlugin:
    ...
    * config bumps a load generation so a stale loader can never install
    ...
    def _spawn_loader_locked(self) -> None:
        ...
        self._load_status = "Preparing obstacle model..."
        ...
        while True:
            with self._state_lock:
                ...
                if cached is not None and cached[0] == effective:
                    return cached[1]
            adapter = _build_distance_adapter(effective, progress_cb=progress_cb)
            stale = None
            with self._state_lock:
                still_wanted = (
    ...
    def _loader(self, generation: int, cfg: dict) -> None:
        ...
        if adapter is None:
            try:
                adapter = _build_distance_adapter(cfg, progress_cb=progress_cb)
        ...
        while True:
            ...
            try:
                node_adapter = self._adapter_for_key(node_key, adapter, progress_cb)
    ...
    def _desc_for(self, state: str, load_error: str | None) -> str:
        ...
        if state == "loading":
            return self._load_status or "Loading obstacle models and TensorRT engines..."
