...
class Processor:
    ...
    def capture(self) -> dict:
        with self._operation_lock:
            if not bool(getattr(self._client, "configured", False)):
                return self.error(
                    "vlm_not_configured",
                    "请先通过 Navigation 卡片配置 VLM API URL、API Key 和模型。",
                    action="capture",
                )
    ...
    def navigate(self, query, *, control_nav_id=None) -> dict:
        ...
        with self._operation_lock:
            ...
            if self._goal_handler is not None:
                planner_result = self._goal_handler(
                    goal,
                    control_nav_id=control_nav_id,
                )
                if not isinstance(planner_result, dict):
                    return self.error(
                        "planner_response_invalid",
                        "同卡片 Nav2 planner 返回了无效结果。",
                        action="navigate",
                        matched=True,
                        navigation_requested=False,
                    )
                if planner_result.get("status") == "error" or planner_result.get(
                    "state"
                ) == "error":
                    return {
                        **planner_result,
                ...
                return {
                    **planner_result,
                    ...
                    "goal_delivery": "in_process_planner",
                    "query": query,
                    "point_id": point.point_id,
                    "description": point.description,
                    "confidence": confidence,
                    ...
                    "message": "已匹配导航点并直接交给同卡片 Nav2 planner。",
                }
            ...
            if not subscriber_ready:
                message = (
                    ...
                    "订阅者；请检查 Navigation 的外部 goal_pose 订阅链路。"
                )
            return {
                "ok": True,
                "action": "navigate",
