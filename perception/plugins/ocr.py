...
TOOLS = [
    {
        ...
        "configSchema": {
            "type": "object",
            "properties": {
                "provider": {"type": "string", "enum": ["local_ppocr", "openai", "qwen", "tesseract"], "description": "OCR 服务商", "scope": "shared"},
                ...
                "model_dir": {"type": "string", "description": "本地模型目录", "scope": "shared"},
                "model_base_url": {"type": "string", "description": "模型下载根地址", "scope": "shared"},
                "num_threads": {"type": "integer", "description": "CPU 推理线程数", "default": 2, "scope": "shared"},
                "rec_score_thresh": {"type": "number", "description": "识别置信度阈值", "default": 0.3, "scope": "instance"},
            },
            "required": ["provider"]
        },
...
def _build_ocr_adapter(cfg: dict) -> Optional[OCRAdapter]:
    """根据配置创建 OCR 适配器"""
    provider = cfg.get('provider', 'local_ppocr')

    if provider == 'local_ppocr':
        from plugins.ocr_local import PPOCRv6ONNXAdapter
        return PPOCRv6ONNXAdapter(cfg)
...
class OCRPlugin:
    ...
    def __init__(self, plugin_cfg: dict, executor):
        ...
        log.info(f"[ocr] plugin init: provider={plugin_cfg.get('provider', 'local_ppocr')}, "
                 f"language={self._language}, "
                 f"key={'set' if plugin_cfg.get('key') else 'MISSING'}")

        if not self._adapter:
            log.warning("[ocr] adapter not configured")
        ...
        if action == "info":
            ...
            if node_key not in self._nodes:
                ...
                if instance_id and instance_id in self._instance_configs:
                    ...
                    inst_adapter = _build_ocr_adapter(instance_config)
                    if inst_adapter:
                        adapter = inst_adapter
                    inst_lang = instance_config.get("language")
                    if inst_lang:
                        language = inst_lang
