"""YOLO26-S metric distance on the shared vision runtime."""
...
from plugins.vision_runtime import VisionDecodeError, VisionEngineSession, decode_depth
...
class MetricDepthBackend:
    ...
    def __init__(self, engine_path: str, config: Mapping[str, object]):
        ...
        self._engine: VisionEngineSession | None = None
    ...
    def _get_engine(self) -> VisionEngineSession:
        if self._engine is None:
            with self._engine_init_lock:
                if self._engine is None:
                    started = time.monotonic()
                    engine = VisionEngineSession(self._engine_path, resize_mode="stretch")
                    try:
                        if engine.input_size != (_INPUT_WIDTH, _INPUT_HEIGHT):
    ...
    def predict_indoor_distance(self, image_bytes: bytes, deadline_monotonic: float) -> float:
        ...
        outputs, meta = self._get_engine().infer(image)
        if len(outputs) != 1:
            raise ObstacleDistanceError(ErrorCode.MODEL_ERROR, "indoor engine must have one output")
        try:
            depth = decode_depth(outputs, meta)
        except VisionDecodeError as error:
            raise ObstacleDistanceError(ErrorCode.INVALID_DEPTH, str(error)) from error
