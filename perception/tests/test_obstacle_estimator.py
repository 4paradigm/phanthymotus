...
def test_backend_overrunning_soft_deadline_returns_timeout():
    clock = iter([0.0, 0.0, 3.0, 3.0])
    backend = SimpleNamespace(predict_indoor_distance=lambda data, deadline: 1.0)
    estimator = ObstacleDistanceEstimator(
        backend, {"soft_timeout_s": 2.5}, monotonic=lambda: next(clock),
    )
    result = estimator.estimate(b"image")
    assert result.status == "error" and result.error_code == "timeout"
    assert result.fallback and result.distance_m == 3.0
