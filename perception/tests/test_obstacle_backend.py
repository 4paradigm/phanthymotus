...
pytestmark = pytest.mark.skipif(
    not hasattr(cv2, "imencode"), reason="image-operator checks require real OpenCV",
)
...
def engine(monkeypatch):
    engine = _Engine()
    monkeypatch.setattr(tensorrt_runtime, "TensorRTEngine", lambda path, **kwargs: engine)
    return engine
...
def test_bad_jpeg_does_not_load_engine(monkeypatch):
    def unexpected(path, **kwargs):
        pytest.fail("invalid image loaded an engine")
    monkeypatch.setattr(tensorrt_runtime, "TensorRTEngine", unexpected)
...
@pytest.mark.parametrize("attribute,value", [
    ("input_shape", (1, 3, 640, 640)), ("output_names", ["a", "b"]),
])
