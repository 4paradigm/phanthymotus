...
class FastLivo2ContractTest(unittest.TestCase):
    ...
    def test_contract_exposes_bounded_relocalization_not_navigation(self) -> None:
        ...
        self.assertIn("view_map.add(candidate_points)", adapter)
        self.assertIn("view_map.add(live_points)", adapter)
        self.assertIn("self._map_view_context.add(", adapter)
        self.assertIn("view_map.add(self._map_view_context.points)", adapter)
