...
**把原始相机也连上去**（可选的 `image/jpeg` 输入），深度色带就以 `view_blend`(0.5) 压在
真实画面上 —— 一张图同时说明「它看见了什么」和「它在对什么做反应」。不连就还是纯深度图，
和以前一样。

融合用一次 `cv2.addWeighted` 定权，不做逐像素 alpha：这段跑在发布 tick 上、紧挨着 10 Hz
的指令流，融合指令在 307k 像素上是零点几毫秒，而带广播掩码的 numpy 表达式是几毫秒。大头
是 JPEG 解码，所以相机帧一直以压缩态存着，只有真要画的时候才解。

**相机话题怎么被认出来**：perception 的深度话题是从相机话题派生的（`<camera>/visual_depth`），
所以一路 `CompressedImage` 只要名字不以那个后缀结尾，就不可能是深度图。认错方向的代价是
不对称的 —— 认成深度图会拿 JPEG 字节去算距离，认成相机则只是在启动时响亮地报「深度没连
上」。
