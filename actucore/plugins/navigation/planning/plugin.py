"""ActuCore bundle adapter for the Nav2 card."""
...
class Nav2Plugin:
    ...
    def dispatch(self, name: str, args: dict) -> dict | None:
        ...
        if not canvas_started or core is None:
            return self._error(
                "canvas_not_started",
                "connect FAST-LIVO2 odom, registered_cloud and static_map, then start Canvas",
            )
    ...
    def _build_backend(self):
        if self._cfg.get("shadow_only") is not True:
            return UnavailableNavigationBackend(
                "ActuCore navigation refuses non-shadow configuration"
            )
