...
`velocity_smoother` 不在 proposal 执行链上，已通过校验 hash 的最小
...
DWB 与 rotation shim 已由卡片内的 `segmented_controller` 取代；
...
`nav2_rviz_plugins`、`nav2_collision_monitor`、`nav2_system_tests`、DWB 和
rotation shim —— 它们会把
