...
def validate_metadata(value: object) -> dict:
    ...
    if base_to_camera is None or (
        isinstance(base_to_camera, dict)
        and base_to_camera.get("status") == "unavailable"
    ):
        t_base_camera = None
        base_transform_source = "unavailable"
        base_transform_error = "calibration_unavailable:base_to_camera"
    else:
