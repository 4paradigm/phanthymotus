...
和三个运行时补丁哈希；`actucore/Dockerfile.navigation-base` 在同一构建中
...
- `runtime/livox_ros_driver2_msgs/` 只保留 Livox ROS Driver2 的
  ...
  运行配置使用标准 `sensor_msgs/PointCloud2`，不启动 Livox Driver，也不再
  拉取或编译 Livox SDK2。
- Sophus:
  `strasdat/Sophus@de0f8d3d92bf776271e16de56d1803940ebccab9`，MIT。
- `runtime/fast_livo2/` 下的 ActuCore adapter 使用 Apache-2.0。
