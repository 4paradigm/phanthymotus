...
1. 展开 **安装 PICO 与一键连接**，点击 **生成安装链接**，查看固定入口 `<Core 地址>/pico`、分组安装码、版本和二维码。安装码 15 分钟有效、最多下载三次（含重试），重新生成会撤销旧码；Core 重启保留剩余有效期。
2. 在 PICO 系统浏览器打开入口、输入安装码，点击“安装遥操”，在下载列表打开 APK 并确认系统安装。手机扫码只打开手机网页，不能自动安装到头显。
3. 安装完成直接点 **打开**；在电脑端 **连接与配对 → 允许新设备配对** 打开窗口。头显选择自动发现的机器人，两端核对配对码并确认。配置自动保存，无需返回安装网页；窗口过期可重新打开。
4. 已有 App 仍可使用一次性连接邀请：在卡片生成邀请，在头显浏览器打开更新后的链接，展开“已有 App，使用连接邀请”并导入。邀请 15 分钟有效、单次兑换、可撤销，只授权配对；新邀请使旧邀请失效。邀请路径继续验证机器人证书。
