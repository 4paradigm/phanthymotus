...
> **版本说明：本文操作步骤对应既有三卡实现。** 设备接入迁往 Driver ext_vr 的 [四卡方案与验收契约](../../../docs/plans/teleop-four-card-contract.md) 已确定但尚未交付；新方案所有安装、配对和设备设置位于 ext_vr 齿轮页，teleop 只负责双臂使能、映射与编排。请勿把以下旧操作和历史结果当作四卡已可用；后续按 PR 编号条目验收。
