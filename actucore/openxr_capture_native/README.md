...
当前源码的 Gradle 包版本与 Native 握手版本均为 `0.3.20-operator1-ikview2`，`versionCode = 24`。此版保留安装邀请，支持三卡架构的 `end_effectors / eef_pose` 任务分配。0.3.18 在配对后点击开始时会拒绝该能力字段并反复重连，需要升级客户端。本手册描述源码行为，不将版本号、宿主测试或 APK 构建视为本轮设备安装和真机验收的证据。
...
1. 电脑端 teleop 卡片生成安装链接，显示固定入口 `<Core 地址>/pico`、12 位安装码和二维码。入口可收藏；安装码分三组，不含容易混淆的 `0/1/I/O`，大小写或输入时的横线、空格不影响新码识别。
2. 在 **PICO 系统浏览器**打开入口、输入安装码，点击“安装遥操”。在浏览器下载列表打开 APK，按系统提示确认安装。手机扫码只打开手机网页；头显系统扫码能力仍需现场验证。
3. 安装器中直接点“打开”。电脑端打开“允许新设备配对”；App 自动发现同一局域网的机器人。选择机器人、核对两端配对码并确认，连接资料自动保存，**无需回安装网页**。配对窗口过期时在电脑端重新打开即可。
4. 已配对应用以后冷启动自动连接。地址输入、更换设备等收在“连接帮助”中；不会凭自动发现结果自动信任或替换已配对机器人。

安装码是仅下载 APK 的约 59 位随机能力，15 分钟有效，最多三次下载；在卡片生成新码使旧码失效。旧 16 字符下载链接仍可使用。固定入口不会列出机器人或公开配对凭据；身份授权仍通过原有双端确认或独立的一次性邀请。

已有应用也可继续使用原来的连接邀请：Canvas 生成邀请 → 头显浏览器打开邀请页 → 展开“已有 App，使用连接邀请” → 导入邀请并确认。邀请 15 分钟有效、单次兑换、可撤销，仅保存在页面/应用内存中。此备用路径继续验证证书指纹，不能覆盖另一台机器人的身份。

首装仍需用户确认安装来源、系统权限或安全边界。安装页、安装码和 Android 连接页是源码变更；清单指定的已发布 APK 不会因源码修改自动更新。维护者须按下文构建流程用既有 release 签名生成、发布 APK 并更新清单，再进行 PICO 浏览器下载、系统安装、直接打开、配对和升级保留凭据的完整真机验收。本变更不重新签发密钥或替换运行中头显的应用。
...
先按 [卡片手册](../plugins/teleop/README.md#从-canvas-开始一次遥操)配置模式，将遥操卡片连到对应 Driver，再开启 Canvas 智能控制。Shadow 只计算和显示，Live 才可能取得 Driver 执行权。双手柄侧面的握把共同使能；松开任意握把即保持，再次握住时由服务端按当前手柄和实测机器人姿态建立相对基准。扳机是连续输入，是否输出手部动作取决于机型和标定配置。

天轶服务声明操作能力时，透视页显示三个中文按钮。用任一手柄射线指向按钮并扣一下扳机：

| 按钮 | 客户端行为与服务端职责 |
|---|---|
| 开始遥操 | Canvas 智能控制已开启、服务端返回 `operator.armed: true` 且双握把松开时才能请求；由 ActuCore 执行标定和会话准备 |
...
服务已连接但 `operator.armed` 为 false 时，三个按钮仍显示；开始按钮置灰，并提示“请先开启智能控制”。结束与立即停止仍可请求。旧服务未提供 `armed` 或字段不是布尔值时按 false 处理；重连先清除此状态，等待服务端新状态。开启智能控制时若扳机已按住，需要松开再点击，不会自动开始。解析和点击检查是离线客户端验证，不代表本轮 APK 已安装。
