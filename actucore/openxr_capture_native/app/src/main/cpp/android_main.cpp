...
constexpr char kAppVersion[] = "0.3.16-operator1-ikview2";
...
// Explicit local-only capture: no configuration, transport, pairing or robot session.
void RecordInputOnly(android_app* app, AndroidLifecycle& lifecycle) {
  OpenXrCapture xr;
  xr.Initialize(app);
  FrameState state;
  ...
  std::optional<std::chrono::steady_clock::time_point> triggered;
  const auto armed = std::chrono::steady_clock::now();
  const auto id = std::chrono::duration_cast<std::chrono::nanoseconds>(armed.time_since_epoch()).count();
  bool completed = false;
  Log(ANDROID_LOG_INFO, "LOCAL_RECORD_ARMED: release both grips, then hold and move for 10 seconds; NO NETWORK");
  while (!app->destroyRequested && !xr.exit_requested()) {
    int events = 0;
    ...
    if (triggered && now - *triggered >= std::chrono::seconds(10)) { completed = true; break; }
    if (!triggered && now - armed >= std::chrono::seconds(120)) break;
    if (!xr.session_running()) { std::this_thread::sleep_for(std::chrono::milliseconds(10)); continue; }
    FrameSample sample;
    if (!xr.RenderFrame(&sample)) continue;
    if (!lifecycle.resumed || !xr.focused()) {
      state = MarkDeadmanReleased(state);
      if (triggered) break;
      continue;
    }
    auto result = BuildFrameV1(state, sample, configuration);
    state = result.state;
    const auto ns = std::chrono::duration_cast<std::chrono::nanoseconds>(now.time_since_epoch()).count();
    const auto row = "{\"received_ns\":" + std::to_string(ns) + ",\"frame\":" + SerializeFrameV1(result.frame) + "}";
    if (!triggered) {
      if (!result.frame.deadman) { previous = row; continue; }
      triggered = now;
      if (previous) rows.push_back(*previous);
      Log(ANDROID_LOG_INFO, "LOCAL_RECORD_STARTED: 10 seconds");
    }
    if (rows.size() >= 2000) break;
    rows.push_back(row);
  }
  const std::string path = std::string(app->activity->internalDataPath) + "/input-" + std::to_string(id) + ".jsonl";
  std::ofstream file(path, std::ios::out | std::ios::trunc);
  for (const auto& row : rows) file << row << '\n';
  file.flush();
  if (!file) throw std::runtime_error("local recording write failed");
  file.close();
  std::ofstream manifest(path + ".result.json");
  manifest << "{\"schema\":\"motus.teleop.input-only.v1\",\"robot_connected\":false,\"hardware_output\":false,\"duration_s\":10,\"complete\":"
           << (completed ? "true" : "false") << ",\"frames\":" << rows.size() << "}\n";
  manifest.flush();
  if (!manifest) throw std::runtime_error("local recording result write failed");
  Log(ANDROID_LOG_INFO, std::string("LOCAL_RECORD_") + (completed ? "COMPLETE " : "INCOMPLETE ") + path + " frames=" + std::to_string(rows.size()));
}
...
void android_main(android_app* app) {
  ...
  try {
    ...
    if (IntentExtra(environment, app->activity->clazz, "record_input_only") == "10") {
      RecordInputOnly(app, lifecycle);
    } else {
    ...
    while (app->destroyRequested == 0 && !xr.exit_requested()) {
      ...
      } else if (link == CaptureLinkState::kFaulted ||
                 std::chrono::steady_clock::now() - disconnected_since > std::chrono::seconds(15)) {
        // Return to the normal launcher instead of hiding a failed connection in passthrough.
        break;
      }
  ...
  if (app != nullptr && app->activity != nullptr) {
    // finish() is asynchronous. NativeActivity.onPause/onDestroy synchronously
    // wait for android_native_app_glue acknowledgements. Returning here before
    // APP_CMD_DESTROY kills their consumer and deadlocks the Java UI thread.
    // The capture resources and stack-owned lifecycle have already unwound.
    ...
    while (!app->destroyRequested) {
      int events = 0;
      android_poll_source* source = nullptr;
      ALooper_pollOnce(50, nullptr, &events, reinterpret_cast<void**>(&source));
