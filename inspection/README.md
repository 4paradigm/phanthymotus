...
- `robot_id` 和 `source_device_id` 都由运行环境自动识别，WebUI 不显示输入框，也不接受用户覆盖。`device_id` 只作为旧配置兼容别名保留。
- `robot_id` 优先使用部署层提供的机器人厂家 SN；当前上海 G1 尚未暴露 Unitree SN，因此明确回退为 `jetson-<serial>`，并通过 `robot_identity_source=jetson-module-serial`、`robot_identity_is_manufacturer_serial=false` 说明它不是机器人厂家 SN。
- 外接设备优先使用 USB 厂家序列号；没有序列号的设备使用品牌、VID/PID 和 USB 物理端口组成稳定 ID。内置麦克风使用机器人作用域复合 ID；同类设备无法唯一匹配时使用 topic 复合 ID，并在 metadata 中保留 identity source。
...
- 删除媒体、metadata 或过期 corrupt 诊断文件后，会从日期目录向上清理空目录；后台每小时额外清扫历史空目录；
