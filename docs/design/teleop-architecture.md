...
> **适用范围：以下保留 #259 既有三卡实现与历史验证。** 2026-09-23 起，后续实现及用户/雨强验收以 [四卡契约](../plans/teleop-four-card-contract.md) 和最新 PR 正文为准：设备接入迁到 Driver ext_vr，motion 反馈直接供 teleop/ext_vr 使用，天轶与北京 G1 并行。新方案尚未实现或验收，本页旧部署记录不表示当前设备状态。
...
天轶禁执行部署下真实关节反馈、EEF 求解、显示回传及 Canvas 三卡 Shadow 启停已通过；随后按用户要求配置三卡 Live 验收，原接待图另行保留。北京 PICO 经批准迁移 release 签名，邀请预填、一键配对、冷启动重连及透视静态模型通过。2026-09-23 开始失败排查确认旧客户端拒绝末端能力字段并反复重连，修复版 0.3.19 已通过 ADB 覆盖安装、保留配对，空闲连接正常；本轮停止了旧输入会话，没有启动机器人动作。准备重试修复及新容器 APK 仍待服务切换。浏览器安装、修复后的手柄动态模型、真实遥操和收臂验收尚未完成。此前天轶实体 PICO 跟随、松握保持与结束收臂记录属于旧链路，不能代替本轮验证。仅迁移天轶双臂；不增加手腿控制，不迁移或删除既有 G1、VLA 和旧协议能力。
