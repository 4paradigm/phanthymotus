...
标准 JP6.1 Dockerfile 已自动包含遥操依赖与 DDS 参考文件，但标准 `actucore/deploy/service.yml` 只描述普通服务挂载，不自动生成站点专属 TLS、配对状态及管理密钥。启动时 Bundle 检查站点配置，缺少或无效则不注册遥操工具，保留字段级 `required_site_config` 诊断及其他卡片。天轶统一候选已补齐该站点挂载；重新构建公共镜像不会凭空获得现场身份，也不会将不可操作的遥操卡片宣称为就绪。
