...
本轮源码、APK 与离线验证已交付，天轶禁执行部署下真实关节反馈、EEF 求解、显示回传及 Canvas 三卡 Shadow 启停通过；原接待图和卡片配置已保留恢复，项目停止。实体北京 PICO 经用户批准迁移至 release 0.3.18，邀请预填、一键配对、冷启动重连及透视静态模型通过；安装使用 ADB，新版浏览器安装、手柄动态模型、真实遥操和收臂验收尚未完成。此前天轶实体 PICO 跟随、松握保持与结束收臂记录属于旧链路，不能代替本轮验证。仅迁移天轶双臂；不增加手腿控制，不迁移或删除既有 G1、VLA 和旧协议能力。
