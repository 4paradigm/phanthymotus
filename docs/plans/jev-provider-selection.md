...
用户已明确要求部署：提交本次 Core 自声/第三人门禁与服务选择改动，推送 PR #254，申请带测试的构建与最新代码审查；通过后只切换天轶 Core。沿用 [部署步骤](tianyi-core-jev-deployment.md)，切换前实时复查空闲和控制隔离，备份 Compose/数据库/容器基线，部署后核验其他服务未变。不自动填写或切换服务凭据、不开始智能控制。
