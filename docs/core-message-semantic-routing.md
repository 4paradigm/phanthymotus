...
单 worker，等待队列最多 8 条。从 Core 接收候选消息开始计算唯一截止时间，预算为 `jev_timeout_s`（最大 5 秒）；排队、上下文准备和至多一次重判共用预算，每次 HTTP 请求只获得剩余时间，不重新计时。请求超时或总预算耗尽时，语音和文字均回退消费时的默认 interrupt mode（steer / interrupt / followup），不因超时 reject，也不使用迟到或旧的模型模式。精确回声规则仍优先拒绝；有效模型 ignore、非超时接口错误、身份不可读、队列满及停止/配置失效的原有策略不变。超时回退不是模型确认：旁人聊天或未匹配回声可能被接入。这不是后续 Core 处理或扬声器停播的时延承诺。uncertain 回退默认模式，结构无效则语音拒绝、文字回退。结果绑定当时 turn，避免使用迟到模型结果中断新任务。
...
- Activity 的 `semantic_routing` 区分判断结果与 `dispatch` 实际分流；当前可能出现 `classified`、`ignore`、`robot_echo`、`invalid_route`、`uncertain_default`、`text_default`、`budget_expired`、`queue_full`、`project_stopped` 或错误类型。旧版本的 audience/addressed 原因不再产生。拒绝判断带 `actual=reject`；完全匹配播报的快速拒绝带 `method=recent_speech_exact`。实时推送全局最多每秒一次，突发期间合并为最新一条，`coalesced` 表示本次合并省略的条数；不是逐消息审计流。即使客户端很慢，也只保留一个推送任务和一个待发样本，不堆积任务。检查具体消息的判断/分流应读取上述接口的最近 100 条诊断；旧记录会被覆盖，需要现场留证时及时读取。
...
未来若有等价模型的国内低时延 API，且当前耗时的重要部分确实来自跨境链路，预计可大幅降低这段前置判断等待及长尾超时，在保留过滤和智能路由优势的同时改善响应。当前 `api_ms` 未分解网络、服务端排队与推理，尚无国内端点实测、上线时间或加速比例保证；仅国内网关转发海外推理不保证改善。应以同输入/模型、同设备的 p50/p95/p99、超时率和 ASR→实际出声链路验证，不承诺 500 ms 端到端。后续 AEC、将本地回声过滤与 Jev 开关解耦均为独立优化，不包含于本次实现。
