...
<details><summary>诊断与维护</summary><div class="tp-maintenance tp-pair-buttons"></div><pre></pre></details>`;
  ...
  for(const [key,def] of Object.entries(configSchema.properties||{})){
    ...
    input.addEventListener('input',()=>{configDirty=true;});label.append(input);configFields.set(key,input);panel.querySelector('.tp-config-fields').append(label);
  }
  if(!saveConfig || !configFields.size)panel.querySelector('.tp-config').hidden=true;
  function showConfig(){
    if(configDirty)return;
    const values={...savedConfig,...info?.configuration};
    for(const [key,input] of configFields)input.value=values[key]??configSchema.properties[key].default??'';
  }
  configForm.addEventListener('submit',async e=>{
    e.preventDefault();if(configForm.querySelector('button').disabled || !configForm.reportValidity())return;
    const values={};for(const [key,input] of configFields){if(!input.value.trim())continue;values[key]=configSchema.properties[key].type==='number'?Number(input.value):input.value.trim();}
